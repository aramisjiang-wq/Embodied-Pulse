...
# 生成更多GitHub项目（已有100个，需要900个）
echo "生成900个GitHub项目..."
node -e "
const { PrismaClient } = require('@prisma/client');
const userPrisma = new PrismaClient({ datasources: { db: { url: 'file:./prisma/prisma/dev.db' } } });
...
echo "📊 数据补充完成！"
echo "当前数据统计："
sqlite3 prisma/prisma/dev.db "SELECT 'papers: ' || COUNT(*) FROM papers UNION SELECT 'videos: ' || COUNT(*) FROM videos UNION SELECT 'repos: ' || COUNT(*) FROM github_repos UNION SELECT 'models: ' || COUNT(*) FROM huggingface_models UNION SELECT 'news: ' || COUNT(*) FROM news UNION SELECT 'jobs: ' || COUNT(*) FROM jobs;"
